...
它不是聊天软件。它更像一层本地“翻译层”：先把明文加密成二进制消息，再用本地 llama.cpp worker 把二进制 payload 藏进一段中文载体文章。对方复制整段文章回来，程序从载体文本恢复 payload，再解密成明文。
...
- 支持按联系人或群聊保存聊天记录，加密和解密成功时自动记录明文。聊天记录正文以逐条加密 BLOB 存入 SQLite；会话类型、会话标识、时间戳、nonce、tag 和消息 UUID 等元数据仍是明文，不是 SQLCipher 式整库加密。
...
1. A 打开“密钥交换”，进入私聊密钥交换，导出密钥文本发给 B。
...
1. 打开“密钥交换”，进入群聊。
2. 创建群时填写本地群名和自己在群里的昵称，导出群密钥文本。
3. 其他成员在群聊模式下导入群密钥文本，也设置本地群名和自己的群昵称。
4. 之后在主窗口顶部选择群聊对象，按普通消息一样加密和解密。
5. 备注名只影响本机显示，用来给群内成员设置本地称呼。

“聊天记录”按钮会显示当前联系人或群聊下保存的明文记录。记录首行包含时间和发送人：
...
群消息使用独立的对称群传输格式，不复用私聊消息格式。普通群消息固定开销为 31 bytes，不包含外层中文载体文本开销。
